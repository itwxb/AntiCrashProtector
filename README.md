...
- `Cannot invoke "it.unimi.dsi.fastutil.objects.ObjectArrayList.get(int)" because "this.wrapped" is null` ⭐ **核心修复目标**
...
- 🛡️ **三层防护体系**：**v1.2.0 重构**！数据包拦截 + 玩家退出检查 + 定时监控，全方位防护。
- 📦 **属性数据包拦截**：**v1.2.0 新增**！在服务器发送属性更新包前检查并修复损坏数据，防止 MMO 插件导致的崩溃。
- 🚀 **玩家退出预检查**：**v1.2.0 新增**！在玩家断开连接前检查并修复属性，防止断开时的崩溃。
- 🔬 **属性修饰符深度修复**：不仅检查属性值，还通过反射深度修复 `fastutil ObjectOpenHashSet` 的 `wrapped is null` 损坏问题。
...
- ⏲️ **高性能分摊负载**：自动监控任务采用分摊 tick 策略，每 tick 仅检查 5 个玩家，性能消耗约 1%，即便百人服也毫无压力。
...
├── AttributePacketInterceptor.java # 数据包拦截器 - 属性包检查修复 (v1.2.0 新增)
...
| **AttributePacketInterceptor** | 属性数据包拦截、发包前检查修复 | `register()`, `onPacketSending()`, `checkAndRepairPlayerAttributes()` |
...
# 数据包拦截设置 (需要 ProtocolLib)
# 此功能可在服务器发送属性更新包前检查并修复损坏数据
# 是防止 MMO 插件导致属性崩溃的核心功能
packet-interception:
  enabled: true
...
monitoring:
  enabled: true
  # 检查间隔（ticks），建议 20-100。数值越小扫描越频繁
  # 设为 20 可每秒检查一次，设为 100 则每5秒检查一次
  check-interval: 20
  checks:
    location: true     # 坐标有效性
...
1. **安装 ProtocolLib**（推荐）：下载 [ProtocolLib](https://www.spigotmc.org/resources/protocollib.1997/) 并放入 `plugins` 文件夹。
2. 将 `AntiCrashProtector-1.2.1.jar` 放入服务器的 `plugins` 文件夹。
3. 重启服务器。
4. 使用 `/anticrash status` 确认所有模块已生效：
   - 属性包拦截: 运行中
   - 自动监控: 运行中
   - 指令拦截: 运行中
...
- **🚀 传送前属性检查**：新增 `PlayerTeleportEvent` 监听器，在玩家传送前检查并修复属性，防止 `/tpa`、`/tpaccept` 等传送命令导致的崩溃。
- **🔄 传送后延迟检查**：传送完成后延迟 3 tick 再次检查，覆盖传送过程中发生的属性损坏。
- **👤 玩家加入后检查**：玩家加入后延迟 20 tick 再次检查，确保所有数据加载完成后再验证属性完整性。
- **🐛 修复问题**：之前 `/tpaccept` 只检查执行者，没有检查被传送者，导致被传送者属性损坏时崩溃。

### v1.2.0 (2026-03-04)

**重大更新：三层防护体系重构**

- **🛡️ 属性数据包拦截**：新增 `AttributePacketInterceptor` 模块，在服务器发送属性更新包前检查并修复损坏数据，从源头防止 MMO 插件导致的崩溃。
- **🚀 玩家退出预检查**：在玩家断开连接前检查并修复属性，防止断开连接过程中的崩溃。
- **🔬 反射深度修复**：通过 Java 反射直接修复 NMS 内部损坏的 `fastutil ObjectOpenHashSet`，清空或置空损坏的修饰符集合。
- **📊 状态命令增强**：`/anticrash status` 现在显示属性包拦截器的状态。
- **⚡ 性能优化**：检查间隔从 100 ticks 改为 20 ticks，更快发现并修复问题。
- **📦 ProtocolLib 集成**：新增 ProtocolLib 软依赖，支持数据包级别的防护。
