...
| **PlayerMonitor** | 玩家数据诊断、分级修复、安全坐标缓存、玩家退出预检查 | `diagnosePlayer()`, `handleCorruptedPlayer()`, `repairCorruptedAttributes()`, `preCheckAndRepairAttributes()` |
...
```java
// 通过反射直接修复 NMS 内部损坏的 fastutil 集合
...
Field modifiersField = nmsAttributeInstance.getClass().getDeclaredField("modifiers");
